...
- AI 用 `dice.request { expr, label, dc, why }` 发起（**label 必填**：必须写明这次投的是什么，例如「敏捷豁免（火球术）」）。
- 发起后玩家面板顶部出现要求条，**投掷按钮亮起（呼吸光效）**：
  `📣 DM 要求你投：敏捷豁免（火球术）（DC 13）　1d20+2　半掩体后躲避爆炸`
- 玩家点一下 → 物理骰盘动画 → 落稳后自动 `dice.answer` 写进战斗记录（**AI 读得到**，DC 自动判成功/失败）。
- **没有要求时按钮不可用**（显示「等待 DM 发起投骰要求…」），玩家不能自己掷判定骰再报数字。

**② 记录（最新在上面）**

- 面板里「战斗判定记录」按时间**倒序**排列，每条显示：时刻 · **用途（label）** · 骰式与结果 · DC 成败。
  这样一眼就能看到刚投的是"为了什么"。

**③ 随手丢（玩具）**

- d4/d6/d8/d10/d12/d20/d100 自由投掷，带重力/回弹/骰间碰撞/翻滚的物理动画，落稳揭晓点数。
- **不写记录、不计入任何统计**，只留在本地"随手丢的"列表里，纯好玩。
- 说明：物理引擎负责翻滚过程，点数由加密随机数预先决定（和真骰子一样，投前就定了）。
### 施法范围模板 + 视线/掩体
